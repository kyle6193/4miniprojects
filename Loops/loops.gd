...
func _process(delta: float) -> void:
	if stars_spawned < spawn_count:
		time_passed += delta # Add time since last frame
...
func spawn_star():
	# Instantiates and configures a new star
	var star = star_scene.instantiate() # Create a new star instance
	add_child(star) # Add the star to the scene tree
